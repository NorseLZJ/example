...
def task(code: str, name: str):
    try:
        ...
        dfa = df[df["date"] > "2024-01-01"]
        idx_min = dfa["close"].idxmin()
        dfa = dfa[dfa["date"] > dfa.loc[idx_min]["date"]]
        dfa.reset_index(drop=True, inplace=True)
        if dfa["close"].idxmin() < dfa["close"].idxmax():
            max_value = dfa["close"].max()
            min_value = dfa["close"].min()
